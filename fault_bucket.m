...
%
% ====== Inputs ======
% x: the input variables for the data e.g time
% y: output variables
% params: structure defining various parameters: cov_fn_name,
%       lookahead, window_length, fault_prior, hyperparams,
%       training_window, fault_sd_ratio, num_fault_sd_samples, 
%       num_norm_sd_samples. See code below for details.
%
% ====== Outputs ======
% x, y: note that the first portion of data is reserved for training, and 
%       hence are not tested on: x and y are the observations used in 
%       testing.
% xstars: x values predicted for
% predictive_means, predictive_variances: predictions for y
% fault_post: posterior probability of faultiness of each y
% mean_fault_sd: posterior for the fault standard deviation hyperparameter
% mean_norm_sd: posterior for the normal standard deviation hyperparameter
%
% Michael Osborne 2013, mosb@robots.ox.ac.uk
...
% cov_fn function, defaults to matern cov_fn with \nu = \frac{5}{2}
% plus iid Gaussian noise
if isfield(params,'cov_fn_name')
    cov_fn_name = params.cov_fn_name;
else
    cov_fn_name = 'covMatern5iso';
end
...
% exp(full_hyper(end)) is the noise variance
% other elements are as per cov_fn chosen. If the default covMatern5iso,
% exp(full_hyper(1)) is the characteristic length scale
% exp(full_hyper(2)) is the signal variance.
if no_training
...
if ~no_training
    ...
    % starting pt for optimisation of output scale
    ...
    % starting pt for optimisation of input scale
    if isfield(params,'trial_input_scale')
        trial_input_scale = params.trial_input_scale;
    else
        trial_input_scale = range(x_train)/10;
    end

    % starting pt for optimisation of noise sd
    if isfield(params,'trial_noise')
        trial_noise = params.trial_noise;
    else
        trial_noise = 1e-1 * trial_output_scale;
    end

    cov_fn = {'covSum', {cov_fn_name, 'covNoise'}};
    % train hyperparameters
    full_hyper = minimize(...
        log([trial_input_scale; trial_output_scale; trial_noise]), ...
        'gpr', -1000, cov_fn, x_train(:), y_train(:));
...
K = @(x) feval(cov_fn{:}, hyper, x);
K2 = @(x, xstar) K_wrapper(cov_fn{:}, hyper, x, xstar);
...
for i = 2:(length(x) - lookahead)
   ...
   if length(xa) >= window_length
       ...
       for ind = 1:num_samples;
           ...
           D_a = (R_a)'\ya;
            ...
            invW_a_half = R_a\D_a;
            invW_a = invW_a_half*invW_a_half';
            gp.hypersamples(ind).invW_a = invW_a;
       end
   ...
   for ind = 1:num_samples;
        ...
        % covariance of eqn 10 in appendix
        cov_term =  Kb - T_ab2 ...
                    + Kab'*invW_a*Kab...
                    - mean_b_a^2;
        cov_term = - T_ab2;
        ...
        %M_b = inv(post_noise_fa*inv(V_fb - Kab' * ) + 
        %       (1-post_noise_fa)*inv(mat_n))
        M_b = cov_b_a_f * ((  (1-post_noise_fb)*cov_b_a_f ...
                            + (post_noise_fb)*cov_b_a_n...
                            )\cov_b_a_n);
